...
- `docker run --privileged` is **approximated, not fully supported** — Apple Container has no privileged mode. `--privileged` grants all capabilities (`cap-add ALL`, the same approximation apple/container's own builder uses; this is what BuildKit's `docker-container` buildx driver needs), but the other things Docker's privileged mode implies — host device access, disabled seccomp/AppArmor — have no Apple Container equivalent. Prefer granular `--cap-add` / `--cap-drop` (and `--read-only`, `--sysctl`) where possible.
